...
class Decoder(nn.Module):
    ...
    def forward(self, input):
        ...
        out = interpolate(output, size=(self.input_size, self.input_size), mode="bilinear", align_corners=True)
...
class LEDNet(nn.Module):
    def __init__(self, input_size, num_classes, encoder=None):
        super().__init__()
...
if __name__ == '__main__':
    ...
    in_size = 512
    net = LEDNet(num_classes=2)
    net.cuda(0)
    summary(net, (3, in_size, in_size))

    in_img = torch.rand((1, 3, in_size, in_size)).cuda(0)
    out_img = net(in_img)
    print(out_img.size())